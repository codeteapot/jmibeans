...
import com.github.codeteapot.jmibeans.PlatformEventTarget;
...
public class PlatformEventQueue implements PlatformEventTarget {
   ...
   *
   * <p>
   * The same listener can be added more than once. The same event will be listened to by a listener
   * as many times as it has been added.
   *
   * <p>
   * Events that have not yet been dispatched will be listened to by the added listener.
   *
   ...
   *
   * <p>
   * This takes into account that the same listener can be added more than once. That is, only one
   * is removed, the rest will remain listening.
   *
   * <p>
   * Events that have not yet been dispatched will no longer be listened to by the removed listener.
   *
   ...
   *
   * <p>
   * Each event involves a call to the corresponding operation of all added listeners at the time it
   * is processed.
   *
   * <p>
   * This is a blocking operation. It ends when the thread in which it is executed is interrupted.
   *
   * <p>
   * This operation can safely be called more than once. The same event will only be processed in
