 ...
 *
 * <p>
 * Changes in the availability of infrastructure machines are known through the provided
 * {@link PlatformEventTarget}. The provided {@link MachineCatalog} is responsible for defining the
 * profile of these machines.
 *
 * <p>
 * The application has consistent access to available machines across all listened
...
public class PlatformAdapter {
  ...
  private final MachineContainer machineContainer;

  /**
   * Adapter that uses the specified event target and catalog.
   *
   * <p>
   * Initially, the adapter is not listening to any port. To start listening on a port, the
   * {@link #listen(PlatformPort)} operation must be called, passing the instance of that port.
   *
   * <p>
   * Building a machine can be a blocking operation, so the executor of the task that involves it is
   * specified.
   *
  ...
  PlatformAdapter(
    ...
    machineContainer = containerConstructor.construct(eventTarget, catalog, builderExecutor);
  }
   ...
   *
   * <p>
   * This is a blocking operation. It ends when the thread responsible for listening to the port is
   * interrupted. A new thread must be used so that the adapter keeps listening on more than one
   * port.
   *
   * <p>
   * The possibility of listening to the same instance more than once is left to the port
   * implementation.
   *
   * <p>
   * When listening to a port ends, the machines that would have been made available to the platform
   * context by the port are lost.
   *
