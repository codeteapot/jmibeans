...
class ManagedMachineStateChangerTest {
  ...
  private static final Set<Runnable> SOME_DISPOSE_TASKS = new HashSet<>();
  ...
  void available(
    ...
    when(availableStateConstructor.construct(
        ...
        SOME_REF,
        someEventTarget,
        SOME_FACETS, SOME_DISPOSE_TASKS)).thenReturn(someAvailableState);

    stateChanger.available(SOME_REF, someEventTarget, SOME_FACETS, SOME_DISPOSE_TASKS);
