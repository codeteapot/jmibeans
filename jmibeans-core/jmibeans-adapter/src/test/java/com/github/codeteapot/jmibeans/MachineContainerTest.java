...
class MachineContainerTest {
  ...
  void setUp() {
    ...
    verify(catalog).registerBuilderPropertyConverters(builderPropertyConverters);
  }
  ...
  void acceptWithProfileRepeated(
      ...
      @Mock MachineBuilderProperties anyBuilderProperties,
    ...
    when(anyLink.getBuilderProperties())
        .thenReturn(anyBuilderProperties);
    ...
    assertThat(machineMap).containsEntry(SOME_REF, someMachine);
    verify(machineConstructor, never()).construct(any(), any(), any(), any());
  }
