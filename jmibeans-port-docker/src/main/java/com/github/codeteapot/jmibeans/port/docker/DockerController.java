    ...
    switch (event.getType()) {
        ...
        switch (event.getAction()) {
          case ACTION_CREATE:
            if (monitorMap.containsKey(monitorId)) {
              ...
              monitorMap.put(monitorId, new DockerMonitorImpl(monitorId, machineManager));
            }
            break;
          case ACTION_DESTROY:
            ...
            ofNullable(monitorMap.get(monitorId))
                .map(runnable(monitor -> {
                  ...
                  containerMapper.map(event.getId())
                      ...
                      .map(runnable(container -> monitor.start(
                          getRoleLabelValue(container)
                              .flatMap(profileResolver::fromRole)
                              .orElseGet(profileResolver::getDefault),
                          new DockerMachineAgent(container))))
      ...
      default:
        logger.finest(new StringBuilder()
            .append("Ignoring event of type ").append(event.getType())
            .toString());
    }
  }
