...
# dnssec-keygen -a HMAC-MD5 -b 512 -n USER ns-rndc.key
# Not possible. Not enough entropy :(
RUN cp /etc/bind/rndc.key /etc/bind/ns-rndc.key

RUN chown -R bind:bind /var/lib/bind
RUN chmod -R g+w /var/lib/bind

RUN chown -R bind:bind /var/log/bind
RUN chown -R bind:bind /var/cache/bind
RUN chown -R bind:bind /etc/bind

RUN useradd bind-adm -m -g bind -s /bin/bash
